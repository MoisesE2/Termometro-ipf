...
export default function Home() {
  ...
  const [cotasArrecadadas, setCotasArrecadadas] = useState(0);
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        const data = (await response.json()) as { totalQuotas?: number; totalReceived?: number };
        setValorArrecadado(Number(data.totalReceived ?? 0));
        setCotasArrecadadas(Number(data.totalQuotas ?? 0));
      } catch {
        // fallback para 0 se API não estiver disponível
      }
    };
