...
export async function donationRoutes(app) {
    ...
    app.patch('/donations/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
        ...
        if (!parse.success)
            return reply.code(400).send({ message: 'Payload inválido.', errors: parse.error.flatten() });
        const data = {};
        if (parse.data.donorName !== undefined)
            data.donorName = parse.data.donorName;
        if (parse.data.quotaCount !== undefined)
            data.quotaCount = parse.data.quotaCount;
        if (parse.data.amountPaid !== undefined)
            data.amountPaid = parse.data.amountPaid;
        if (parse.data.paymentDate !== undefined)
            data.paymentDate = parse.data.paymentDate;
