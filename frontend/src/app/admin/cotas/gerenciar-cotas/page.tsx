...
export default function GerenciarCotasPage() {
  ...
  const loadCotas = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3001/api/cotas/relatorio', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/cotas/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleUpdate = async (cotaAtualizada: Cota) => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/cotas/${cotaAtualizada.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
